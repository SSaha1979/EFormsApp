...
class Test_008_ApproveLVL3:
    ...
    def test_login(self,setup):
        self.logger.info("***************Test_008_Approval_LVL3***************")
        self.logger.info("***********Verifying Approval_LVL3 test*************")
        self.driver = setup
        self.driver.get(self.baseURL)
        ...
        if self.driver.find_element(By.ID, "approveSubmit"):
            assert True
            self.driver.close()
            self.logger.info("***********Approval_LVL3 test is passed**************")
        else:
            self.driver.save_screenshot(".\\Screenshots\\" + "test_NewFormApprvdlvl3.png")
            self.driver.close()
            self.logger.error("************Approval_LVL3 test is failed************")
            assert False
